...
\noindent Osservando il filmato campione ci si aspetterebbe di osservare un miglioramento nelle condizioni di visibili\`a, cosa non evidente dai valori restituiti. Tali incongruenze potrebbero essere imputabili a possibili variazioni della velocit\`a nel veicolo usato per l'acquisizione del filmato, in violazione delle ipotesi fatte in partenza. Per questo motivo l'esperimento \`e stato ripetuto utilizzando i nuovi filmati acquisiti.\\

\noindent Il filmato \`e stato ripreso con una videocamera a 30fps su di un veicolo alla velocit\`a costante di 50 km/h (precisione ottenibile con un tachimetro analogico) in condizioni di visibilit\`a ideali, e da questi sono state estratte diverse sequenze di immagini da massimo cinquanta frames. Ciascuna sequenza \`e stata analizzata dieci volte.\\

\begin{table}[H]
\centering
\begin{tabular}{|c|c|c|}
	\hline
	$\lambda$ medio	& deviazione standard	& visibilit\`a a 50 km/h \\ \hline
	1.36s		& 0.31s		& 18.9m \\ \hline
	1.33s		& 0.072s	& 18.4m \\ \hline
\end{tabular}
\end{table}

\noindent I risultati appaiono essere congruenti, dal momento che le condizioni nei vari spezzoni possono essere considerate identiche. \\
...
\noindent In modo analogo al caso precedente, l'algoritmo \`e stato eseguito dieci volte per diversi frame di partenza e numero di immagini, con risultati difficilmente interpretabili.
