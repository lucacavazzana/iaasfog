...
\begin{minipage}[t]{0.5\linewidth}
	...
	feat23 - $k: , \lambda:  $\\
	\includegraphics[scale=\imFeat]{images/feat23}\\
\end{minipage}
\begin{minipage}[t]{0.5\linewidth}
	\centering
	feat20 - $k: , \lambda:  $\\
...
%	feat24 - $k: , \lambda:  $\\
%	\includegraphics[scale=\imFeat]{images/feat24}\\
\end{minipage}
\caption[short]{Features e relative curve fittate. I dati sono ordinati in base al tempo d'impatto.}
\label{fig:feats4}
\end{figure}
...
\noindent Sono infine stati analizzati i diversi valori di $\lambda$ restituiti dagli algoritmi al variare del numero di frame analizzati e dal frame di partenza; ogni test \`e stato eseguito 10 volte per verificare anche la ripetibilit\`a dei risultati a causa della componente stocastica introdotta da RANSAC nella determinazione del punto di fuga e nelle elaborazioni successive.\\
...
\begin{tabular}{|c|c|c|c|c|}
	...
	\hline
\end{tabular}
\caption{Numero di frame medi estratti dalla sequenza di immagini. Sulle righe l'immagine di partenza, sulle colonne il numero di frame analizzati.}
\label{tabNum}
\end{center}
\end{table}
