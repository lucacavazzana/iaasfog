...
\noindent La visibilit\`a viene espressa come una distanza: tale misura potrebbe essere calcolata mediante il birapporto note le posizioni di un numero di landmark presenti nella scena o mediante il tempo all'impatto data la velocit\`a del veicolo. Sfortunatamente tali informazioni per il video campione disponibile non sono disponibili, quindi ci si \`e limitati a considerare il tempo d'impatto medio fornito dal parametro $\lambda$ come valutazione delle condizioni di visibilit\`a nell'ipotesi (forte) di velocit\`a costante.\\
...
\noindent I test sono stati eseguiti su una sequenza di 113 immagini estratte da un video acquisito mediante una telecamera fissata su un veicolo. Per ovviare a diverse incognite sono state fatte diverse assunzioni:
\begin{itemize}
...
\end{tabular}
\caption{Campioni estratti dalla sequenza di immagini analizzate. Si noti come le condizioni di visibilit\`a migliorano.}
\label{fig:video}
\end{figure}
...
\noindent In modo analogo al caso precedente, l'algoritmo \`e stato eseguito dieci volte per diversi frame di partenza e numero di immagini.
...
\section{Ulteriori test}
\nonindent Allo scopo di meglio verificare le prestazioni  del codice sviluppato si \`e deciso di tentare ulteriori test utilizzando nuovi filmati, questa volta misurando tutti i parametri mancanti. Sfortunatamente, essendo la nebbia in questa stagione di difficile reperibilit\`a, ci si \`e dovuti accontentare di un test in condizioni di visibilit\`a quasi ideali.\\

\subsection{Tempi d'impatto}
\noindent Allo scopo di determinare l'affidabilit\`a dei tempi d'impatto calcolati si \`e deciso di sviluppare un test per confrontarli con quelli reali: una volta selezionato un insieme di features inseguite correttamente ne viene calcolato il tempo d'impatto rispetto al punto in cui \`e stato acquisito il primo frame (uguale al tti calcolato in una qualsiasi immagine $i$ pi\`u il tempo di frame moltiplicato per $i-1$), valore che verr\`a confrontato con quello effettivo ottenuto dividendo la distanza reale dell'elemento che ha generato la feature per la velocit\`a del veicolo.\\

\newcommand{\ttiScale}{0.25}
\begin{table}[H]
\centering
\begin{tabular}{|c|c|c|c|c|}
	\hline
	& distanza & tti reale & tti calcolato al frame 0 & errore\\
	\hline
	\includegraphics[scale=\ttiScale]{images/tti1.png} & 32m	& 2.88s	& 2.72s	& 0.16s	\\ \hline
	\includegraphics[scale=\ttiScale]{images/tti2.png} & 24m	& 2.16s	& 2.22s	& 0.06s	\\ \hline
	\includegraphics[scale=\ttiScale]{images/tti3.png} & 46m	& 4.14s	& 5.08s	& 0.96s	\\ \hline
	\includegraphics[scale=\ttiScale]{images/tti4.png} & 19m	& 1.71s	& 1.9s	& 0.19s	\\ \hline
	\includegraphics[scale=\ttiScale]{images/tti5.png} & 20m	& 1.8s	& 2.08s	& 0.28s	\\ \hline
\end{tabular}
...
\begin{tabular}{|c|c|c|c|c|}
	...
	& distanza & tti reale & tti calcolato al frame 0 & errore\\
	\hline
	\includegraphics[scale=\ttiScale]{images/tti6.png} & 64m	& 5.76s	& 3.86s	& 1.9s	\\ \hline
	\includegraphics[scale=\ttiScale]{images/tti7.png} & 46m	& 4.14s	& 4.32s	& 0.18s	\\ \hline
	\includegraphics[scale=\ttiScale]{images/tti6.png} & 64m	& 5.76s	& 5.34s	& 0.42s	\\ \hline
\end{tabular}
\caption{Valori ottenuti viaggiando ad una velocit\`a di 40 km/h. In giallo la feature inseguita.}
\end{table}
...
\begin{tabular}{|c|c|c|c|}
	\hline
	distanza & tti reale & tti calcolato al frame 0 & errore\\ \hline
...
\noindent I risultati per i livelli di precisione dei valori considerati (precisione del tachimetro analogico nel misurare la velocit\`a, precisione con cui \`e stata ricostruita la posizione del veicolo nel primo frame, precisione delle distanze misurate) possono considerarsi ampiamente soddisfacienti.\\

\subsection{Visibilit\`a}
\label{sec:vis}

\noindent Data una sequenza di 50 frame ripresi a 30fps su di un veicolo a 50 km/h (con la precisione ottenibile con un tachimetro analogico) ed analizzata per dieci volte mediante il metodo di fitting di $\lambda$ ha restituito un valore di 1.36 (con deviazione standard 0.31s), corrispondente ad una visibilit\`a di 18,9m.\\
\noindent Un secondo filmato acquisito nella corsia opposta rispetto al precedente (in condizioni simili quindi) ed analizzato per dieci volte ha decretato un $\lambda$ di 1.33 (deviazione standard 0.072s), corrispondenti a 18.4m. I risultati sembrano dunque essere congruenti.\\

\noindent Similmente, dieci iterazioni di RANSAC sul primo filmato hanno calcolato un $\lambda$ medio di 1.41s con deviazione standard 0.22s, mentre sul secondo una media di con deviazione standard .\\

\noindent Ricordiamo che il $\lambda$ ottenuto corrisponde alla distanza media per cui la curva teorica del contrasto delle varie features raggiunge il valore $k_f/e$, scelto in modo da semplificare i calcoli, e non va quindi interpretato come visibilit\`a in termini ``umani'': per far ci\`o si dovrebbe determinare un valore percentuale $q$ sul contrasto raggiunto il quale una feature campione pu\`o essere considerata visibile per un occhio umano e semplicemente riscalare i valori ottenuti mediante un coefficiente $\alpha$ (pari a $-log\left(q\right)$) come mostra la seguente formula:

$$qk_f = k_fe^{-t_{imp,q}/\lambda}$$
$$t_{imp,q} = -\lambda\log \left(q\right) = \alpha\lambda$$

\noindent ricordando che $q$ \`e inferiore all'unit\`a e quindi $\alpha$ risulter\`a essere positivo. Calcolato il $\lambda$ come mostrato fin'ora, non rimarr\`a che moltiplicarlo per $\alpha$ per ottenere la distanza cui la feature assume il livello $qk_f$ per cui possiamo considerarla come visibile.\\
...
\noindent I valori ottenuti negli esperimenti in condizioni di visibilit\`a ideale sono comunque hanno mostrato un certo grado di consistenza degli algoritmi realizzati. Rimane tuttavia necessario testarli in condizioni nebbia avendo a disposizione le corrette informazioni a disposizione.\\
...
\noindent \`E inoltre necessario uno studio su come manipolare il $\lambda$ restituito in modo da conferirgli un significato pi\`u vicino al mondo delle percezioni umane, pi\'u intuitivo per l'utente, in modo simile a come suggerito alla fine della sezione \ref{sec:vis}.\\
